      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card>
          <div className="flex items-center gap-2 mb-4">
            <Layers size={18} className="text-rail-400" />
            <h2 className="font-semibold">Configuration</h2>
          </div>
          <div className="space-y-5">
            <div>
              <label className="flex items-center justify-between text-sm mb-1.5">
                <span className="text-gray-400">N-Shot</span>
            ...
            <div>
              <label className="flex items-center justify-between text-sm mb-1.5">
                <span className="text-gray-400">N-Way</span>
            ...
            <div>
              <label className="flex items-center justify-between text-sm mb-1.5">
                ...
                <span className="font-mono text-xs text-gray-500">{nEpisodes}</span>
              </label>
              <input
                type="range"
                min={10}
            ...
            <button
              onClick={runEval}
              disabled={loading}
              className="w-full flex items-center justify-center gap-2 py-2.5 bg-rail-600 hover:bg-rail-500 rounded-lg font-medium transition-colors disabled:opacity-50"
            >
              {loading ? (
        ...
        <div className="lg:col-span-2 space-y-6">
          <Card>
            <div className="flex items-center gap-2 mb-4">
              <BarChart3 size={18} className="text-rail-400" />
            ...
            {result ? (
              result.error ? (
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  {[
                    { label: "Mean Accuracy", value: `${(result.mean_accuracy * 100).toFixed(1)}%`, icon: Target, color: "text-rail-400" },
                    { label: "Std Dev", value: `${(result.std_accuracy * 100).toFixed(1)}%`, icon: TrendingUp, color: "text-gray-400" },
                    { label: "95% CI", value: result.ci_95 ? `±${(result.ci_95 * 100).toFixed(1)}%` : "—", icon: BarChart3, color: "text-gray-400" },
                    { label: "Open-Set Detection", value: result.open_set_detection_rate !== undefined ? `${(result.open_set_detection_rate * 100).toFixed(1)}%` : "N/A", icon: Layers, color: "text-yellow-400" },
                  ].map((m, i) => {
                    const Icon = m.icon
                    return (
                      <div key={i} className="bg-gray-900 rounded-lg p-4 text-center">
                        <Icon size={18} className={`mx-auto mb-2 ${m.color}`} />
                        <p className="text-2xl font-bold font-mono text-rail-400">{m.value}</p>
            ...
            <Card>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                <div className="bg-gray-900 rounded-lg p-3">
                  <p className="text-rail-400 font-medium mb-1">Median Prototypes</p>
                  <p className="text-gray-400 text-xs">Class-median rather than mean embedding for robustness against outlier shots</p>
                </div>
                <div className="bg-gray-900 rounded-lg p-3">
                  <p className="text-rail-400 font-medium mb-1">Episodic Training</p>
                  <p className="text-gray-400 text-xs">N-way K-shot episodes with disjoint support/query splits</p>
                </div>
                <div className="bg-gray-900 rounded-lg p-3">
                  <p className="text-rail-400 font-medium mb-1">Open-Set Rejection</p>
                  <p className="text-gray-400 text-xs">Distance-to-prototype threshold prevents forced misclassification</p>
                </div>
              </div>
            </Card>
          )}
